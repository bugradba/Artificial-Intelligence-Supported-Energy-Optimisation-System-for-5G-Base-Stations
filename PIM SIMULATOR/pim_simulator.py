...
class PIM_Core:
    ...
    def __init__(self,technology_node=28):
        #pPIM: A Programmable Processor-in-Memory Architecture With Precision-Scaling for Deep Learning makalesindeki alınan gerçek değerler (28nm)
        self.delay_ns = 0.8 #Nano Saniye
        self.power_mw = 2.7 #Miliwat
        self.lut_size = 256 # 2**8(4 bit x 4 bit input)
        self.teach_node = technology_node

        # LUT initialize (4-bit multiplication tablosu)
        self.lut = self._build_multiplication_lut()

    def _build_multiplication_lut(self):
        """
        4-bit x 4-bit için lookup table.
        Gerçek bir donanımda bu SRAM de tutulur 
        """
    ...
    def multiply_4bit(self, a, b):
        ...
        Returns:
            ...
            energy_pj: Harcanan enerji (picojoule)
            latency_ns: İşlem gecikmesi (nanosaniye)

        """

        # LUT'tan oku (gerçek donanımda tek clock cycle)
        result = self.lut[a, b]
    ...
    def add_4bit(self, a ,b ):
        """
        4-bit toplama (LUT'ta farklı function-word yüklenerek yapılır).
        Çarpımdan daha hızlı ama aynı power consumption.
        """

        result =  (a + b) & 0xFF #8 bit sonuç 
        energy.pj = self.power_mw * self.delay_ns
...
class PIMluster():
    ...
    def mac_8bit(self, a_8bit, b_8bit, accumulator=0, precision=8):
        ...
        """
         ...
         return result & 0xFFFF, total_energy, total_latency
...
class PIMArray:
    ...
    def __init__(self, num_clusters=256, technology_node=28):
        self.clusters = [PIMluster(technology_node) for _ in range(num_clusters)]
    ...
    def convolution_layer(self, input_shape, kernel_shape, precision=8):
        ...
        macs_per_cluster = total_macs // self.num_cluster

        #Tek bir mac maliyet hesabı 
        dummy_cluster = self.cluster[0] 
        ...
        total_latency_ms = (macs_per_cluster * mac_latency_ns) / 1e6  # ns -> ms
        return {
            'precision': precision,
